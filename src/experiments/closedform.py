...
def get_lambda_block(t, xt, x1prime, prior_log_density, eps=1e-8):
    """
    t:       (Bx, 1) or (Bx,)
    xt:      (Bx, 256, 32)
    x1prime: (Bp, 256, 32)
    ...
    x0 = (xt - t[:, None, None, None] * x1prime) / torch.clamp(1 - t[:, None, None, None], min=eps)  # (Bx, Bp, 256, 32)
    logp0 = prior_log_density(x0)           # should return (Bx, Bp)
    return logp0


def u_star(t, xt, x1prime_loader, prior_log_density, eps=1e-8):
    """
    Parameters:
    t: batch_size * 1
    ...
    valid_rows = ~torch.isinf(all_priors).all(dim=1)
    assert valid_rows.any(), "All rows in all_priors are -inf, cannot compute softmax"
    assert not torch.isnan(all_priors).any(), "NaN values found in all_priors, cannot compute softmax"
    ...
    for x1prime in x1prime_loader:
        ...
        ucond = (x1prime[None, :, :, :] - xt[:, None, :, :]) / torch.clamp(1 - t[:, None, None], min=eps)  # (Bx, batch_size_mean, 256, 32)
        u_tot += (lambda_batch * ucond).sum(dim=1)  # sum over batch_size_mean
        processed += batch_size_mean
    return u_tot


def u_star_with_labels(t, xt, x1prime_loader, labels, prior_log_density, eps=1e-8):
    """
    Parameters:
    t: batch_size * 1
    xt: batch_size * size_img
    ...
    for x1prime_batch, label_batch in x1prime_loader:
        ...
        lambda_batch = all_lambdas[:, processed:processed + batch_size_mean]  # (Bx, batch_size_mean)
        lambda_batch = lambda_batch[:, :, None, None]  # (Bx, batch_size_mean, 1, 1)
        ucond = (x1prime_batch[None, :, :, :] - xt[:, None, :, :]) / torch.clamp(1 - t[:, None, None], min=eps)  # (Bx, batch_size_mean, 256, 32)
        u_tot += (lambda_batch * ucond).sum(dim=1)  # sum over batch_size_mean
        processed += batch_size_mean
    return u_tot
