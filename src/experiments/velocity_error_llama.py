...
from experiments.closedform import u_star, u_star_with_labels
...
def main():
    ...
    checkpoint = torch.load("checkpoints/llamagen_epoch_250.pt", map_location=device)
    ...
    n_xt_samples = 128
    batch_size_xt = 8
    batch_size_x1prime = 256
    ts = torch.linspace(0, 1.0 - clamp_t, n_steps, device=device)

    # Create dataloader of sequences of codebook vectors (not indices)
    # DO NOT USE SHUFFLE for x1' dataloader !
    if PROBS:
        print("[setup] Using one-hot encoded dataloader for xt and x1prime.")
        dataloader_xt = get_q_loader_with_labels(batch_size=batch_size_xt, codebook_size=codebook_size, shuffle=True)
        if USE_LABELS:
            dataloader_x1prime = get_q_loader_with_labels(batch_size=batch_size_x1prime, codebook_size=codebook_size, shuffle=False)
        else:
            dataloader_x1prime = get_q_loader(batch_size=batch_size_x1prime, codebook_size=codebook_size, shuffle=False)
    else:
        print("[setup] Using codebook vector dataloader for xt and x1prime.")
        dataloader_xt = get_vec_dataloader_with_labels(batch_size=batch_size_xt, codebook=vq_model.quantize.embedding.weight, shuffle=True)
        if USE_LABELS:
            dataloader_x1prime = get_vec_dataloader_with_labels(batch_size=batch_size_x1prime, codebook=vq_model.quantize.embedding.weight, shuffle=False)
        else:
            dataloader_x1prime = get_vec_dataloader(batch_size=batch_size_x1prime, codebook=vq_model.quantize.embedding.weight, shuffle=False)
    ...
    with torch.inference_mode():
        for i, t in enumerate(ts):
            ...
            for x1_b, labels in pbar:
                ...
                x0_b = vfm_wrapper.sample_gauss_prior(xt_b_size, device=device)
                xt_b = x0_b * (1 - t) + x1_b * t
                t_b = t * torch.ones(xt_b_size, 1, device=device)

                # Compute closed form velocity
                if USE_LABELS:
                    u_star_b = u_star_with_labels(t_b, xt_b, dataloader_x1prime, labels, vfm_wrapper.gauss_prior_log_density) # shape: (batch_size, seq_len, codebook_dim)
                else:
                    u_star_b = u_star(t_b, xt_b, dataloader_x1prime, vfm_wrapper.gauss_prior_log_density) # shape: (batch_size, seq_len, codebook_dim)

                # Compute model velocity
                with torch.no_grad():
                    if PROBS:
                        model_velocity_b = vfm_wrapper.prob_velocity(t_b.reshape(-1), xt_b, is_inference=True, cond_idx=labels if USE_LABELS else None) # shape: (batch_size, seq_len, codebook_size)
                    else:
                        model_velocity_b = vfm_wrapper.velocity(t_b.reshape(-1), xt_b, is_inference=True, cond_idx=labels if USE_LABELS else None) # shape: (batch_size, seq_len, codebook_dim)
